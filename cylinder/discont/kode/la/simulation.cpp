...
void Simulation::step(){
  ...
  for (int i=0; i<I; ++i){
    for (int j=0; j<J; ++j){
      const la::Vector<6> uij = u_ij[i+1][j+1];
...
//    / std::max(1., norm(uij_prev, "inf"));
  double shift = (u_ij[i+1][j+1] - uij_prev).norm()
    / std::max(1., uij_prev.norm());
  ...
  if (istream.is_open()){
    while (getline(istream,line)){
      ...
      while (getline(ss, su, ';')){
        ...
        la::Vector<6> _u = la::make_vec<6>(u);
        u_ij[i+1][j+1] = _u;
        u.clear();
        k=0;
        ++i;
...
const la::Matrix<6>& Simulation::diffr_coeff(int i, int j){
...
const la::Matrix<6>& Simulation::diffz_coeff(int i, int j){//??? 
...
const la::Matrix<6>& Simulation::C_ij(int i, int j){
  return C_i[i];
}
const la::Matrix<6>& Simulation::A_ij(int i, int j){
  return A_i[i];
}
const la::Matrix<6>& Simulation::F_ij(int i, int j){
...
const la::Matrix<6>& Simulation::G_ij(int i, int j){
  /* same questions here */
  if (j > 0){
    if (i > rmti){
      return G_i[1];
...
const la::Matrix<6> E_mx = la::make_mat(
           { la::make_vec<6>({ 1, 0, 0, 0, 0, 0})
           , la::make_vec<6>({ 0, 1, 0, 0, 0, 0})
           , la::make_vec<6>({ 0, 0, 1, 0, 0, 0})
           , la::make_vec<6>({ 0, 0, 0, 1, 0, 0})
           , la::make_vec<6>({ 0, 0, 0, 0, 1, 0})
           , la::make_vec<6>({ 0, 0, 0, 0, 0, 1})
});
const la::Matrix<6> diffusionMask = la::make_mat(
           { la::make_vec<6>({ 1, 0, 0, 0, 0, 0})
           , la::make_vec<6>({ 0, 1, 0, 0, 0, 0})
           , la::make_vec<6>({ 0, 0, 1, 0, 0, 0})
           , la::make_vec<6>({ 0, 0, 0, 0, 0, 0})
           , la::make_vec<6>({ 0, 0, 0, 0, 0, 0})
           , la::make_vec<6>({ 0, 0, 0, 0, 0, 0})
});
const la::Matrix<6> dbulk_mx = dbulk * diffusionMask;
const la::Matrix<6> dmt_mx = dmt * diffusionMask;
const la::Matrix<6> dav_mx = dav * diffusionMask;
const la::Matrix<6> dzer_mx = 0 * diffusionMask;
