...
extern const la::Matrix<6> E_mx;
extern const la::Matrix<6> diffusionMask;
extern const la::Matrix<6> dbulk_mx;
extern const la::Matrix<6> dmt_mx;
extern const la::Matrix<6> dav_mx;
extern const la::Matrix<6> dzer_mx;
...
private:
  ...
  using vec = la::Vector<n>;
  template <std::size_t dim>
  using mat = la::Matrix<dim>;
...
protected:
  Gri2hd<mat<6>> BJL_ij = Gri2hd<mat<6>>(I, J);
private:
  /* time-dependent */
  Gri2hd<mat<6>> BJF_ij = Gri2hd<mat<6>>(I, J);
  Gri2hd<mat<6>> B_ij = Gri2hd<mat<6>>(I, J);
  Gri2hd<mat<6>>Binv_ij = Gri2hd<mat<6>>(I, J);
  /* time-independent */
  std::vector<mat<6>> C_i = std::vector<mat<6>>(I);
  std::vector<mat<6>> A_i = std::vector<mat<6>>(I);
  mat<6> F_i[3];
  mat<6> G_i[3];
  //1D->2D
protected:
  const mat<6>& C_ij(int i, int j);
  const mat<6>& A_ij(int i, int j);
  const mat<6>& F_ij(int i, int j);
  const mat<6>& G_ij(int i, int j);
  ...
  const mat<6>& diffr_coeff(int i, int j); // diffr_coeff(i,j) stands for d_i,j+0.5
  bool z_in_mt(int i, int j);
  bool z_in_bulk(int i, int j);
  //bool z_in_av(int i, int j);//not called
  bool z_in_vanish(int i, int j);
  const mat<6>& diffz_coeff(int i, int j); // diffz_coeff(i,j) stands for d_i+0.5,j
private:
